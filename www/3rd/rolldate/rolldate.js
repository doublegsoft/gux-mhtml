...
.rolldate-container .rolldate-clear{
    left:0;
    right:atuo;
    font-weight: bold;
    color:var(--color-error);
...
@media screen and (max-width:320px){
  ...
  Rolldate.prototype = {
    ...
    createUI: function createUI() {
      ...
      for (var i = 0; i < len; i++) {
        ...
        if (f == 'YYYY') {
          ...
          for (var o = 0; o < config.values.length; o++) {
            let item = config.values[o];
            itemClass = item.value == this.selectedValue ? 'active' : '';
            ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex + '" data-value="' + item.value + '">' + item.text + '</li>';
            domMndex++;
          }
        }
        ul += '</ul></div>';
      ...
      var $html = `
        ...
        <div class="rolldate-panel">                
          <header>                    
            <span class="rolldate-btn rolldate-clear">清除</span>
            ${lang.title}                   
    ...
    event: function event() {
      ...
      _this.tap(clear, function () {
        var config = _this.config;
        if (config.confirm) {
          var flag = config.confirm.call(_this, '');
          if (flag === false) {
            return false;
          } else if (flag) {
            date = flag;
      ...
      _this.tap(confirm, function () {
        ...
        if (config.confirm) {
          var flag = config.confirm.call(_this, newDate);
          if (flag === false) {
            return false;
          } else if (flag) {
            date = flag;
    ...
    getSelected: function getSelected(scroll) {
      if (this.config.format == 'oo') {
        return {
          text: $('#' + scroll.wrapper.id + ' li', 1)[scroll.getSelectedIndex()].innerText,
          value: $('#' + scroll.wrapper.id + ' li', 1)[scroll.getSelectedIndex()].getAttribute('data-value'),
        };
      }
